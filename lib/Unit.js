...
function Unit({ arg, targets, operations, loaders = {}, config = {}, predicates = [], silent = false, name }) {
    ...
    if (isParallel(arg)) {
        ...
        const unit = Unit({ arg: parallel, targets, operations, loaders, config, predicates, silent, name });
    ...
    if (isName(arg)) {
        ...
        if (isOperation(conformed)) return [ Operation({ operation: conformed, targets, operations, loaders, config, predicates, silent }) ];

        return Unit({ arg: targets[conformed], targets, operations, loaders, config, predicates, name: conformed, silent });
    }

    if (isTarget(arg)) return [ Target({ fn: arg, config, predicates, name, silent }) ];
