...
function Operation({ operation, targets, operations, loaders, config, predicates }) {
    ...
    if (hasTag(__fn__, 'target')) {
        const target = __fn__(...args);
        registerLabel(target.label || target.name);
        return target;
    }
...
Operation.isBindShorthand = (v) => argsOnly(v) && isOperation(v); // TODO: consider if operation exists so we can simplify base merge syntax when using short-hand ... i.e. enable use of "@config.profile-a" instead of "@config.profile-a::"
