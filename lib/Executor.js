...
const { getLatest:getLatestConfig, set:setConfig } = ConfigStore;
...
function Executor({ fn, name, ns, config:boundConfig }) {
    if (get(getLatestConfig(), `__not_when__.${name}`, false)) return Promise.resolve();
    if (get(getLatestConfig(), `__not_when_next__.${name}`, false)) {
        setConfig(`__not_when_next__.${name}`, false);
