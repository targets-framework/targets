...
exports.ScheduledUnit = ScheduledUnit;
...
const { setting: { get:getSetting } } = Store;
...
const DashboardUpdate = () => modes[getSetting('mode')].updateDashboard;
const QueueReportUpdate = () => modes[getSetting('mode')].updateQueueReport;
    ...
    if (isArray(unit)) {
        return (hasTag(unit, 'parallel'))
            ...
            : unit
                ...
                .reduce(async (chain, fn) => Promise.resolve([
                    ...(await chain),
                    await fn()
                ]), Promise.resolve([]));
    } else {
