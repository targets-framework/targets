...
const bind = (fromPath, ...rest) => {
    if (rest.length === 2) {
        const [ toPath, { config: { set }, results: { getLatest } } ] = rest;
        return set(toPath, get(getLatest(), fromPath));
    } else {
        const [ { config: { baseMerge }, results: { getLatest } } ] = rest;
...
const When = (ns) => (fromPath, targetName, { config: { set }, results: { getLatest } }) => {
